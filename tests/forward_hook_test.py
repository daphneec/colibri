...
#
# Created:
#   09 Nov 2023, 10:43:17
# Last edited:
#   09 Nov 2023, 13:11:05
# Auto updated?
#   Yes
#
# Description:
#   File to test if the forward hook fix defined in `utils/fix_hook.py`
#   actually works.
#
...
##### FUNCTIONS FROM PROJECT #####
# These, together, are how and why we need the forward hooks
...
class DummyNetwork(cnn.Module):
    """
        A dummy neural network á lá crypten.
    """

    def __init__(self):
        """
            Constructor that just initializes _something_.
        """
        ...
        # *any* module to test
        self.conv1 = cnn.Conv2d(1, 6, 5)

    def forward(self, x):
        """
            Runs the forward pass of the dummy network.
        """

        x = cnn.MaxPool2d(cnn.ReLU(self.conv1(x)), (2, 2))
        return x
...
if __name__ == "__main__":
    ...
    fix_hook(cnn.Module)         # Uncomment to see the magic happen!
    dummy_network = DummyNetwork()

    # Encrypt it and add the profiling thingies
    a = dummy_network.encrypt(src = 0)

    # Add the profiling thing
    add_profiling_hooks(a, 10, verbose=True)

    # Run a forward pass and observe
    inp = crypten.randn(1, 1, 32, 32)
    a.forward(inp)
