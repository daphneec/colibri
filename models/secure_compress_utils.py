...
def compress_conv(m_new, m_old, mask, dim, prefix_new=None, prefix_old=None):
    assert m_new is None or isinstance(m_new, cnn.Conv2d)
    assert isinstance(m_old, cnn.Conv2d)
    assert dim in [0, 1]
...
def compress_bn(m_new, m_old, mask, prefix_new=None, prefix_old=None):
    assert m_new is None or isinstance(m_new, cnn.BatchNorm2d)
    assert isinstance(m_old, cnn.BatchNorm2d)
    ...
    for adjust_info in adjust_infos:
        ...
        with torch.no_grad():
            with crypten.no_grad():
                inp = active_fn(bias[~mask].view(1, -1, 1, 1))
                wgh = following_conv_weight[:, ~mask]
                # tmp = F.conv2d(active_fn(bias[~mask].view(1, -1, 1, 1)),
                #             following_conv_weight[:, ~mask])
                tmp = inp.conv2d(wgh)
                print('adjust', tmp)
                running_mean.data.sub_(crypten.squeeze(tmp))


def adjust_bn(m_new, m_old, post_hook_params, **kwargs):
    mask = crypten.ones_like(m_new.weight, dtype=torch.bool)
...
def copmress_inverted_residual_channels(m,
    ...
    def update(infos):
        for info in infos:
            ...
            if prune_info is not None and issubclass(
                    info['module_class'],
                    cnn.BatchNorm2d) and info['type'] == 'variable':
    ...
    indices = crypten.arange(len(m.ops))
    ...
    new_indices = crypten.arange(len(new_ops))
    if m.expand:
        idx_depth = 1
        idx_proj = 2
    else:
    ...
    for new_op, new_indice, old_op, old_indice, mask in zip(
        ...
        adjust_info['bias'] = _find_only_one(
            lambda info: issubclass(info['module_class'], cnn.BatchNorm2d) and
            'bias' in info['var_old_name'], depth_infos)
        adjust_info['following_conv_weight'] = _find_only_one(
            lambda info: issubclass(info['module_class'], cnn.Conv2d) and
